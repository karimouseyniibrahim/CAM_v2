...
@section('content')
    @component(layoutForm() , ['title' => trans('section.section') , 'model' => 'section' , 'action' => isset($item) ? trans('home.edit')  : trans('home.add')  ])
        ...
        <form action="{{ concatenateLangToUrl('admin/section/item') }}{{ isset($item) ? '/'.$item->id : '' }}" method="post" enctype="multipart/form-data">
 		 ...
 		 <div class="form-group  {{  $errors->has("name.en")  &&  $errors->has("name.ar")  ? "has-error" : "" }}" >
			...
			<div class="form-group {{  $errors->has("description.en")  &&  $errors->has("description.ar")  ? "has-error" : "" }}">
					...
					{!! extractFiled(isset($item) ? $item : null , "description", isset($item->description) ? $item->description : old("description") , "textarea" , "section", "tinymce") !!}
			</div>
			...
			<div class="form-group">
				<div class="form-line">
					...
					@if(isset($item) && $item->image != '')
						<img src="{{ url('/'.env('UPLOAD_PATH').'/Section/'.$item->id.'/'.$item->image) }}" class="img-responsive thumbnail" alt="">
						<br>
					@endif    
					{!! csrf_field() !!}
					<div class="form-group">
            ...
            <div class="form-group">
                <button type="submit" name="submit" class="btn btn-default" >
                    ...
                    {{ trans('home.save') }}  {{ trans('section.section') }}
                </button>
            </div>
        </form>
    @endcomponent
