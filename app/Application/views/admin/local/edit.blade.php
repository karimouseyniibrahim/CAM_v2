...
@section('content')
    @component(layoutForm() , ['title' => trans('local.local') , 'model' => 'local' , 'action' => isset($item) ? trans('home.edit')  : trans('home.add')  ])
        ...
        <form action="{{ concatenateLangToUrl('admin/local/item') }}{{ isset($item) ? '/'.$item->id : '' }}" method="post" enctype="multipart/form-data">
			{{ csrf_field() }}

			<div class="form-group {{ $errors->has("section_id") ? "has-error" : "" }}">
				<label for="section_id">{{ trans("local.section_id")}}</label> 
				@php $sectionName =  isset($item->section_id) ? $item->section_id : old("section_id") @endphp
				{!! Form::select('section_id',$data['sections'],$sectionName, ['class'=>'form-control','id'=>'section_id'])!!}
			</div>
			...
			<div class="form-group">
				<div class="form-line">
					...
					@if(isset($item) && $item->image != '')
						<img src="{{ url('/'.env('UPLOAD_PATH').'/Local/'.$item->id.'/'.$item->image) }}" style="height:200px" class="img-responsive thumbnail" alt="">
						<br>
					@endif    
					{!! csrf_field() !!}
					<div class="form-group">
				...
				<div class="form-group col-md-6">
					<div class="form-group {{ $errors->has("price") ? "has-error" : "" }}" > 
						<label for="price">{{ trans("local.price")}}</label>
							<input type="number" name="price" class="form-control" id="price" value="{{ isset($item->price) ? $item->price : old("price") }}"  placeholder="{{ trans("local.price")}}">
					</div>
					@if ($errors->has("price"))
						<div class="alert alert-danger">
							<span class='help-block'>
				...
				<div class="form-group col-md-6">
					<div class="form-group {{ $errors->has("area") ? "has-error" : "" }}" > 
						<label for="area">{{ trans("local.area")}}</label>
							<input type="number" name="area" class="form-control" id="area" value="{{ isset($item->area) ? $item->area : old("area") }}"  placeholder="{{ trans("local.area")}}">
					</div>
					@if ($errors->has("area"))
						<div class="alert alert-danger">
							<span class='help-block'>
