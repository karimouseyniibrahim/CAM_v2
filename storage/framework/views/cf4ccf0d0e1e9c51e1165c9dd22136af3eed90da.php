<?php echo e(Html::script('admin/plugins/nestable/jquery.nestable.js')); ?>

<?php echo e(Html::script('admin/js/pages/ui/modals.js')); ?>

<?php echo e(Html::script('admin/plugins/bootstrap-notify/bootstrap-notify.js')); ?>

<script>
    var url = "<?php echo e(concatenateLangToUrl('admin/update/menuItem')); ?>";
    ...
    $('#defaultModal').on('shown.bs.modal', function(evt) {
        ...
        if($(evt.relatedTarget).data('url') && $(evt.relatedTarget).data('id')){
            ...
            $.get("<?php echo e(concatenateLangToUrl('admin/getItemInfo/')); ?>/"+id , function(result){
                var json = JSON.parse(result);
                var names = JSON.parse(json.name);
                <?php $__currentLoopData = getAvLang(); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $K => $L): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    $('#<?php echo e('name_'.$K); ?>').val(names.<?php echo e($K); ?>);
                ...
                $('#actionBtn').html('<?php echo e(trans('menu.save_item')); ?>');
            });
        }else{
            clearFields();
            $('#actionBtn').removeAttr('onclick');
            $('#menu_id').val(<?php echo e($item->id); ?>);
            $('#actionBtn').html('<?php echo e(trans('menu.add_item')); ?>');
        }
    });
    ...
    function UpdateItem(){
        ...
        $.post("<?php echo e(concatenateLangToUrl('admin/updateOneMenuItem/')); ?>/",data, function(result){
